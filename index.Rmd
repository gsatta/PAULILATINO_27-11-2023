...
crowns0 <- st_read("./SHP/CHIOME_ANALISI_NDVI_2.shp")
```
...
st_write(crowns, "./SHP/CHIOME_ANALISI_NDVI_2_correct.shp", append = FALSE)
...
for (i in seq_along(crowns_list)) {
  ...
  file_name <- paste0("./SHP/crowns_areas/", area_name, ".shp")
...
ndvi <- terra::rast("./RASTER/ndvi_merged.tif")

# Load the NDVI crowns file
crowns <- st_read("./SHP/CHIOME_ANALISI_NDVI_2_correct.shp")

# set the output folder
output_folder <- "./RASTER/NDVI_CLIPPED"
```
...
raster_folder  <- "./RASTER/NDVI_CLIPPED_NEW"
output_folder <- "./SHP/NDVI_EXTRACTED"
```
...
INPUT_folder <- "./SHP/NDVI_EXTRACTED"
...
output_file3 <- "./SHP/NDVI_VALUES/NDVI_VALUES.shp"
...
NDVI_VALUES <- st_read("./SHP/NDVI_VALUES/NDVI_VALUES.shp")
...
st_write(ndvi_avg_areas, "./SHP/NDVI_VALUES/ndvi_avg_areas.shp", append= FALSE)
```

```{r Compute the mean NDVI_3, include=FALSE}
ndvi_avg_areas <- read_csv("./SHP/NDVI_VALUES/ndvi_avg_areas.shp")
```
...
ndvi_avg0 <- st_read("./SHP/NDVI_VALUES/ndvi_avg_areas.shp")

# Create a new dataframe excluding the 2023 dates
ndvi_avg <- ndvi_avg0 %>% filter(year(date) != 2023)

areas <- unique(ndvi_avg$AREA)
write.csv(areas, file = "./CSV/areas.csv", row.names = FALSE, col.names = TRUE)
...
ggsave("./index_files/figure-gfm/NDVI_means_area1.jpg", plot = p1, dpi = 200, width = 2100, height = 1499, units = "px")
```

![Graph of the mean NDVI for each area (1)](./index_files/figure-gfm/NDVI_means_area1.jpg)
...
# Save the graph in the desired directory
ggsave("./index_files/figure-gfm/NDVI_means_area2.jpg", plot = p2, dpi = 200, width = 2100, height = 1499, units = "px")
```

![Graph of the mean NDVI for each area (2)](./index_files/figure-gfm/NDVI_means_area2.jpg)
...
st_write(crowns, "./SHP/crowns_merged.shp", append = FALSE)
```

```{r Prepare the crowns polygons_2, include=FALSE}
crowns <- read_csv("./SHP/crowns_merged.shp")
```
...
crowns <- st_read("./SHP/crowns_merged.shp", crs = 32632)

NDVI_VALUES_0 <- st_read("./SHP/NDVI_VALUES/NDVI_VALUES.shp", crs = 32632)
...
st_write(filtered_df, "./SHP/NDVI_VALUES/merged_data_ndvi.shp", append= FALSE)
```

```{r Start the elaboration for individual Tree crowns_3, include=FALSE}
filtered_df <- read_csv("./SHP/NDVI_VALUES/merged_data_ndvi.shp")
```
...
NDVI_VALUES <- st_read("./SHP/NDVI_VALUES/merged_data_ndvi.shp")
```
...
# Store the list
saveRDS(results_list, file = "./LIST/results_list.rds")
```
...
results_list <- readRDS("./LIST/results_list.rds")
...
# Store the resuls_df
write_csv(results_df, "./CSV/results_df_class.csv")
```
...
results_df <- read_csv("./CSV/results_df_class.csv")

# View a preview of the data frame
head(results_df)
```

```{r class elements number, warning = FALSE, message = FALSE}
# Compute the number of elements inside each class
count_class <- table(results_df$Trend_Class)

# Calcola la percentuale e formatta la tabella
percent_table <- prop.table(count_class) * 100
simple_percente_table <- round(percent_table, digits = 0)
formatted_table <- cbind(Count = count_class, Percent = simple_percente_table)
...
dataframe <- read_csv("./CSV/results_df_class.csv")
...
# Store the list
saveRDS(dbest_results_list, "./LIST/dbest_results_list.rds")
```

```{r DBEST alghoritm_3, eval = FALSE}
# Load the new list
dbest_results_list <- readRDS("./LIST/dbest_results_list.rds")
...
# Store the list
saveRDS(negative_change_dates, "./LIST/negative_change_dates.rds")

# Load the list of the ndvi negative change dates
negative_change_dates <- readRDS("./LIST/negative_change_dates.rds")

# Load the csv file of th satellite imagies dates
dates <- read_csv("./CSV/date.csv")
...
write.csv(final_dataframe, "./CSV/finaldataframe.csv")
```

```{r DBEST alghoritm_5, message=FALSE, warning=FALSE, echo = FALSE}
# Load the final dataframe
final_dataframe <- read_csv("./CSV/finaldataframe.csv")
...
ggsave("./index_files/figure-gfm/plot.jpg", plot = p)
```

![Graph of the negative change dates (1)](./index_files/figure-gfm/plot.jpg)
...
write_csv(final_dataframe, "./CSV/Weather/final_dataframe.csv" )
...
# Salva il tuo grafico nella directory desiderata
ggsave("./index_files/figure-gfm/plot2.jpg", plot = p)
```

![Graph of the negative change dates (2)](./index_files/figure-gfm/plot2.jpg)
...
class <- read_csv("./CSV/results_df_class.csv", col_names = T)
...
st_write(pixel_trend, "./SHP/NDVI_VALUES/pixels_trend3.shp", append = FALSE)
```

```{r Prepare the files_4, echo = FALSE, message = FALSE, warning = FALSE}
# Load the pixel trend shp file
pixel_trend <- st_read("./SHP/NDVI_VALUES/pixels_trend3.shp")
...
sample_32632 <- st_read("./SHP/FILES/DB_sample_vector.shp")
...
st_write(sample_wgs84, "./SHP/FILES/sample_points.shp", append = FALSE )
```
...
# Load the Paulilatino municipality boundaries shapefile
lim_paul_32632 <- st_read("./SHP/FILES//Limite_Amministrativo_Paulilatino_EPSG-32632.shp")

# Assign the CRS of ex_sf to points
lim_paul_wgs84 <- st_transform(lim_paul_32632, crs = 4326)

# Store the shp file 
st_write(lim_paul_wgs84, "./SHP/FILES/Limite_Amministrativo_Paulilatino_wgs84.shp", append = FALSE )
```
...
focolai0 <- st_read("./SHP/FILES/FOCOLAI.shp")

focolai <- st_transform(focolai0, crs = 4326)

st_write(focolai, "./SHP/FILES/FOCOLAI_wgs84.shp", append = FALSE )
```
...
# Load the plots shapefile
plots <- st_read("./SHP/FILES/BUFFER_ANALISI_NDVI.shp")

# Assign the CRS of ex_sf to points
plots <- st_transform(plots, crs = 4326)

# Store the new shapefile converter to wgs84
st_write(plots, "./SHP/FILES/BUFFER_ANALISI_NDVI_WGS84.shp", append = TRUE)
```
...
In this section, a interactive map is created using the leaflet package. There is a customized palette for visualize different colors in the trend class map. The map is also stored in local.
...
samples <- st_read("./SHP/FILES/sample_points.shp")

# Load the shp file of the administrative boundaries of Paulilatino municipality
lim_paul_wgs84 <- st_read("./SHP/FILES/Limite_Amministrativo_Paulilatino_wgs84.shp")

# Load the shp file of the outbreack in the Paulilatino area
focolai_wgs84 <- st_read("./SHP/FILES/FOCOLAI_wgs84.shp")

# Load the shp file of the plots location
plots_wgs84 <- st_read("./SHP/FILES/BUFFER_ANALISI_NDVI_WGS84.shp")
```
...
saveWidget(map, file = "./HTML/map1.html", selfcontained = FALSE)
```
